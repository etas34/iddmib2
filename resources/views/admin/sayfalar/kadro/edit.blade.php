...
{{--                                @endforeach--}}
                            <select name="kadro" class="form-control">
                                <option value="{{$kadro->kadro}}" selected disabled > {{$kadro->kadro}}</option>
                            </select>

                        </div>
                        <div class="form-group" id="sektor" @if($kadro->sektor_id == 0) style="display: none" @endif>
                            <label>Sektör </label>
                            <select name="sektor_id" id="sektor_id" class="form-control">
{{--                                <option selected disabled value="0">Sektör Seçiniz</option>--}}
{{--                                @foreach($sektor as $key=>$value)--}}
{{--                                    <option value="{{$value->id}}">{{$value->baslik}}</option>--}}
{{--                                @endforeach--}}
                                <option selected disabled> {{\App\Models\Sektor::find($kadro->sektor_id)->baslik ?? ''}}</option>
                            </select>
